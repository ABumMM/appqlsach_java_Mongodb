...
public class TLForm extends javax.swing.JFrame {
    ...
    public void TheLoai_Load() {
        try {
            ...
            MongoCursor<Document> cursor = theLoaiCollection.find().iterator();

            DefaultTableModel model = (DefaultTableModel) listTL.getModel();
            model.setRowCount(0); // Reset bảng

            while (cursor.hasNext()) {
                Document doc = cursor.next();  // Đảm bảo đây là org.bson.Document
                Vector<String> v1 = new Vector<>();

                // Lấy dữ liệu từ document và đưa vào bảng
                v1.add(doc.getString("MaTL"));  // Sử dụng getString để lấy giá trị kiểu String
                v1.add(doc.getString("TenTL"));
                v1.add(doc.getString("MoTaTL"));

                model.addRow(v1); // Thêm hàng vào model
            }

            listTL.setModel(model); // Cập nhật model cho bảng
        } catch (Exception ex) {
            Logger.getLogger(TLForm.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void ReSetActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ReSetActionPerformed
        ...
        LamMoi();
        try {
            // Lấy toàn bộ danh sách thể loại từ MongoDB
            MongoCursor<Document> cursor = theLoaiCollection.find().iterator();
    ...
    private void TimKiemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_TimKiemActionPerformed
        ...
        try {
            // Tìm thể loại theo MaTL trong MongoDB
            Document query = new Document("MaTL", txtMaTL.getText());
            Document result = theLoaiCollection.find(query).first();

            if (result != null) {
                txtMaTL.setText(result.getString("MaTL"));
                txtTenTL.setText(result.getString("TenTL"));
                txtMoTaTL.setText(result.getString("MoTaTL"));
            } else {
    ...
    private void CapNhatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_CapNhatActionPerformed
        ...
        try {
            // Tạo đối tượng Document chứa dữ liệu mới
            Document updatedData = new Document()
                    .append("TenTL", txtTenTL.getText())
                    .append("MoTaTL", txtMoTaTL.getText());

            // Cập nhật thể loại trong MongoDB
            Document query = new Document("MaTL", txtMaTL.getText());
            theLoaiCollection.updateOne(query, new Document("$set", updatedData));
